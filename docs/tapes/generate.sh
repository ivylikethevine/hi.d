...
# renders every tape but `demo` on the self-hosted box and hands the GIFs to
# pages.yml for the site, so those seven are autogenerated and nothing commits
...
# and .githooks/demo_staleness.sh is what says when it has gone stale.
set -euo pipefail
