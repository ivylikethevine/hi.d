...
# real docker path (_say_hi_container) against each of them over `docker
# exec`, not ssh. This proves both branches of _say_hi_container: the
# bash-present main path (tar copy + `bash --rcfile`), and every arm of the
# bash-less fallback's `for s in zsh fish sh` probe.
# Everything is ephemeral and nothing touches host ssh config. Skips cleanly
# if docker isn't installed/running. Needs network access the first time it
# runs, to pull/build the test images (cached by docker afterwards).
set -euo pipefail
...
_HI_WORKDIR="$(mktemp -d -t hi.dockertest.XXXXXX)"
...
_hi_h1 "Testing hi's docker path across container shell environments"
...
_HI_ZSH_OK=1
_HI_FISH_OK=1
_hi_h3 "building hi-dockertest-zsh from $_HI_WORKDIR/zsh (log: $_HI_WORKDIR/zsh.log)"
docker build -q -t hi-dockertest-zsh "$_HI_WORKDIR/zsh" >/dev/null 2>"$_HI_WORKDIR/zsh.log" || _HI_ZSH_OK=0
_hi_h3 "building hi-dockertest-fish from $_HI_WORKDIR/fish (log: $_HI_WORKDIR/fish.log)"
docker build -q -t hi-dockertest-fish "$_HI_WORKDIR/fish" >/dev/null 2>"$_HI_WORKDIR/fish.log" || _HI_FISH_OK=0
...
# docker exec -it refuses to allocate a remote tty unless our own stdin
...
exec 3<&0
_hi_pty_wrap 3 auto "no tty and no python3 to fake one - docker exec -it will fail outright, results may be unreliable"
...
function _hi_container_running() { [ "$(docker container inspect -f '{{.State.Running}}' "$1" 2>/dev/null)" = true ]; }
...
function _hi_run_case() {
  ...
  name="hi-dockertest-$label-$$"
  _hi_h3 "Testing shell: $label"
  t0="$(_hi_now)"

  if ! docker run -d --name "$name" "$image" tail -f /dev/null >/dev/null 2>"$_HI_WORKDIR/$label.run.log"; then
  ...
  if printf '%s' "$out" | grep -q "$_HI_MARKER"; then
    _hi_cecho " | $label -- docker path OK ($(_hi_elapsed "$t0" "$t1")s)" "$GREEN"
  else
...
[ "$_HI_ZSH_OK" -eq 1 ] && _hi_case _hi_run_case zsh hi-dockertest-zsh "$_HI_CMD_FALLBACK"
[ "$_HI_FISH_OK" -eq 1 ] && _hi_case _hi_run_case fish hi-dockertest-fish "$_HI_CMD_FALLBACK_FISH"
_hi_case _hi_run_case sh alpine:3.20 "$_HI_CMD_FALLBACK"

if [ "$_HI_FAILED" -eq 0 ]; then
  _hi_h1 "hi's docker path survived every shell environment tested ($_HI_TOTAL cases)"
else
  _hi_h1 "hi's docker path FAILED: $_HI_FAILED/$_HI_TOTAL cases" "$RED"
fi

docker image rm -f hi-dockertest-zsh hi-dockertest-fish >/dev/null 2>&1 || true
exit "$_HI_FAILED"
