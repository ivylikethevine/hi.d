...
if [ ! -t 3 ]; then
  if command -v python3 >/dev/null 2>&1; then
    ...
    _hi_cecho " | no tty and no python3 to fake one - docker exec -it will fail outright, results may be unreliable" "$YELLOW"
  fi
fi
...
function _hi_wait_for_container() {
  local name="$1" i
  for ((i = 0; i < 40; i++)); do
    [ "$(docker container inspect -f '{{.State.Running}}' "$name" 2>/dev/null)" = true ] && return 0
    sleep 0.25
  done
  return 1
}
...
function _hi_run_case() {
  ...
  if printf '%s' "$out" | grep -q "$_HI_MARKER"; then
    _hi_cecho " | $label -- docker path OK" "$GREEN"
  else
...
[ "$_HI_ZSH_OK" -eq 1 ] && { _hi_run_case zsh hi-dockertest-zsh "$_HI_CMD_FALLBACK" || _HI_FAILED=1; }
[ "$_HI_FISH_OK" -eq 1 ] && { _hi_run_case fish hi-dockertest-fish "$_HI_CMD_FALLBACK_FISH" || _HI_FAILED=1; }
_hi_run_case sh alpine:3.20 "$_HI_CMD_FALLBACK" || _HI_FAILED=1

if [ "$_HI_FAILED" -eq 0 ]; then
  _hi_h1 "hi's docker path survived every shell environment tested"
else
  _hi_h1 "hi's docker path FAILED for one or more shell environments"
fi
