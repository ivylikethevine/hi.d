...
# bootstrap.sh's defaults+settings preamble again, because config.fish's
# `bash -c "source $_HI_SHARED"` reaches this file directly. It can't source
# bootstrap.sh instead: bootstrap sources this file back, and its
# `command -v _hi_cecho` guard can't help mid-definition.
# $_hi_bootstrapped is set by bootstrap.sh and deliberately not exported, so it
# is false exactly when bootstrap didn't run - including in the bash subprocess
# fish spawns. Without it, settings.sh and paths.sh are sourced twice per shell.
if [ -z "${_hi_bootstrapped:-}" ]; then
  : "${_HI_DISABLE_LOCAL:=0}"
