...
| `_HI_PACKAGES_MIN_PRIORITY` | `0`              | the lowest `misc/packages` priority the header's check will print. The file ranks every entry 0-5 and each rank has its own pair of colors, so a rank can already hide its "not installed" half - what it cannot do is show _less_. Set `4` and a session gets a two-line check instead of a paragraph; set it above `5` and the check prints nothing at all rather than a blank line. `hi --packages-preview` marks the ranks it silences `below floor` and counts them under the legend, so the setting is legible before you connect anywhere |
