...
| `tests/compat/targets_test.sh`                  | unit tests for `targets.sh` and `bash.sh`'s completion, against fixture ssh configs and fake backend CLIs  |     |                                               |
| `tests/compat/paths_test.sh`                    | unit tests for `paths.sh`'s local-only gate, both directions, and that settings reach it                   |     |                                               |
| `tests/compat/color_preview_test.sh`            | unit tests for `color_preview.sh`'s precedence, table inputs and layout helpers                            |     |                                               |
| `tests/compat/hi_test.sh`                       | unit tests for `hi.sh`'s argument parsing, backend predicates and generated bootloader/fallback rc         |     |                                               |
...
| `tests/compat/shellcheck_test.sh`               | the lint gate: shellcheck over every `*.sh`, plus `zsh -n`/`fish --no-execute` on the two it can't parse   |     |                                               |
